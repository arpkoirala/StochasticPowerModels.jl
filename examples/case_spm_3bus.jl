...
[data["gen"]["$i"]["pgmin"]= [0, 0, 0] for i=1:length(data["gen"])]
[data["gen"]["$i"]["pgmax"]= [2, 2, 2] for i=1:length(data["gen"])]
[data["gen"]["$i"]["qgmin"]= [-1, -1, -1] for i=1:length(data["gen"])]
[data["gen"]["$i"]["qgmax"]= [1, 1, 1] for i=1:length(data["gen"])]
...
# result = PMD.solve_mc_opf(file, IVRUPowerModel, ipopt_solver)
